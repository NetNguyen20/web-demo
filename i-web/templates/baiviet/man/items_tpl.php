        ...
        <div class="oneOne">
            ...
            <div class="title">
                ...
                <div class="timkiem">
                    ...
                    <?php if($GLOBAL[$com][$type]['trangthai-batdongsan']==true){ ?>

                    <select class="main_select" id="search-idloai-chothue" onChange="window.location.href='index.html?com=<?=$_GET['com']?>&act=man&type=<?=$_GET['type']?>&id_loai='+this.value">

                        <option value="0">Chọn trạng thái</option>

                        <option value="1" <?=($_GET['id_loai']==1) ? 'selected':''?>>Còn hàng</option>

                        <option value="2" <?=($_GET['id_loai']==2) ? 'selected':''?>>Hết hàng</option>
                                ...
                                <td align="center">
                                    ...
                                    <select
                                      ...
                                      <?php if($items[$i]['id_loai']==1){
                                            ...
                                            echo 'style="border-color:#4FEA41"';

                                        }else if($items[$i]['id_loai']==2){

                                            echo 'style="border-color:#FEAC2B"';
                                    ...
                                    name="status" data-table="baiviet" data-type="post" data-box=".main_select<?=$i?>" data-send="id_loai" data-id="<?=$items[$i]['id']?>" class="main_select main_select<?=$i?> js-status-muaban">

                                      <option value="0">

                                        Chọn trạng thái</option>

                                    <option value="1" <?=(@$items[$i]['id_loai']==1) ? 'selected':''?>>

                                            Còn hàng</option>

                                    <option value="2" <?=(@$items[$i]['id_loai']==2) ? 'selected':''?>>
...
<script>
    ...
    $(document).ready(()=>{
        ...
        $('.js-status-muaban').on('change',function(){

            let id = $(this).attr('data-id');

            let value = $(this).val();

            let variable = $(this).attr('data-send');

            let box = $(this).attr('data-box');

            $.ajax({

                url:'ajax/ajaxStatusMuaban.php',

                type:'POST',

                dataType:'json',

                data:{id:id,value:value,variable:variable},
                ...
                success:function(data){
                    ...
                    if(data.status==200){
                        ...
                        $(box).css('border-color',data.color);
