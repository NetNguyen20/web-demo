<?php
  ...
  $totalItem  = $db->rawQueryOne("select count(id) as total from #_baiviet where hienthi=1 and noibat=1 and type=? {$where}",array($type));
  ...
  $query = "select *,tenkhongdau_$lang as tenkhongdau from #_baiviet where hienthi=1 and noibat=1 and type=? {$where} limit $startPoint,$perPage";
